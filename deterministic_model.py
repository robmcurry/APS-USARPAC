...
def solve_deterministic_vrp_with_aps(scenario, time_periods=range(1, 6), vehicle_list=None, P_max=5, M=10000):
    ...
    # --- Scenario Summary Debug Print ---
    print(f"--- Scenario Summary ---")
    print(f"Scenario ID: {scenario.get('scenario_id')}")
    print(f"Impact Radius (km): {scenario.get('type_attributes', {}).get('impact_radius_km')}")
    print(f"Infrastructure Loss Rate: {scenario.get('type_attributes', {}).get('infrastructure_loss_rate')}")
    print(f"Number of Nodes: {len(set(i for (i, _) in scenario['demand']))}")
    print(f"Number of Commodities: {len(set(c for (_, c) in scenario['demand']))}")
    print(f"Total Demand: {sum(scenario['demand'].values())}")
    print(f"Total Supply: {sum(scenario['supply'].values())}")
    print(f"Total Arc Capacity: {sum(scenario['capacity'].values())}")
    print(f"Affected Nodes: {scenario.get('affected_nodes', [])}")
    print(f"------------------------")
    # --- End Scenario Summary Debug Print ---
    ...
    impacted_nodes = set(scenario.get("affected_nodes", []))
    ...
    # 14. SafetyStock (skip enforcement in impacted nodes)
    model.addConstrs((
        alpha[i, c] >= (
            0 if i in impacted_nodes else ell[i, c] - (
                q[i, c] - quicksum(
    ...
    # Prevent APS placement and initial inventory in impacted nodes
    for i in impacted_nodes:
        for c in commodity_list:
            model.addConstr(q[i, c] == 0, name=f"ForceZeroInitialInventory_{i}_{c}")
            model.addConstr(w[i, c, 0] == 0, name=f"ForceZeroInitialW_{i}_{c}")
        model.addConstr(p[i] == 0, name=f"DisallowAPSPlacement_{i}")
    ...
    results = {
        ...
        "aps_locations": [i for i in node_list if p[i].x > 0.5] if model.Status == GRB.OPTIMAL else [],
        "num_q_positive": sum(1 for i in node_list for c in commodity_list if q[i, c].x > 0) if model.Status == GRB.OPTIMAL else 0,
        "num_initial_inventory": sum(1 for i in node_list for c in commodity_list if w[i, c, 0].x > 0) if model.Status == GRB.OPTIMAL else 0,
        "total_flow": sum(flow_summary.values()),
        "status": model.Status,
        "q": {(i, c): q[i, c].x for i in node_list for c in commodity_list if q[i, c].x > 0} if model.Status == GRB.OPTIMAL else {},
        "q_full": {(i, c): q[i, c].x for i in node_list for c in commodity_list} if model.Status == GRB.OPTIMAL else {},
        "initial_inventory": {(i, c): w[i, c, 0].x for i in node_list for c in commodity_list if w[i, c, 0].x > 0} if model.Status == GRB.OPTIMAL else {},
        "w_full": {(i, c, t): w[i, c, t].x for i in node_list for c in commodity_list for t in time_period_list2} if model.Status == GRB.OPTIMAL else {},
