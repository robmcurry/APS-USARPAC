"""
convergence_stage1.py

Stage 1 of the SAA convergence check: solve the alpha=0.5 instance (uniform
degradation-matrix scaling; the post-Step-4 equivalent of the old gamma=0.5
arc-fragility scalar) at a fixed scenario count (N=100) across several
independent scenario draws (seeds) to assess how much the objective value and
site selection vary with the random sample.

Run as `python -m analysis.convergence_stage1` from the aps_usarpac/ project
root. Results are written to output/convergence_stage1.csv (one row per
seed) and the full Gurobi log for each seed is written to
output/convergence_stage1_seed{seed}.log.
"""
...
def main():
    ...
    for seed in SEEDS:
        ...
        # capture the full gurobi log for this seed by redirecting the
        ...
        log_path = os.path.join(OUTPUT_DIR, f"convergence_stage1_seed{seed}.log")
        stdout_fd = 1
        ...
        # M1 memory fix: capture scalars, then dispose the Gurobi model so the
        # next seed's solve doesn't run with the previous model still resident.
        ...
        print(
            f"seed={seed} | status={results.get('status')} | "
            f"objective={results.get('objective_value')} | "
            f"selected_sites={selected_sites} | "
            f"solve_time={solve_time_seconds:.2f}s | "
            f"mip_gap={mip_gap_at_termination:.6f}"
        )

    df = pd.DataFrame(rows)
    out_path = os.path.join(OUTPUT_DIR, "convergence_stage1.csv")
