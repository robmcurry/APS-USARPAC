...
def solve_deterministic_vrp_with_aps_single_stage(scenario, vehicle_list=None, P_max=7, M=9900,
    ...
    node_list = sorted(set(int(i) for i in scenario['region_mapping']))

    # Load region mapping from scenario
    region_mapping = {int(k): int(v) for k, v in scenario['region_mapping'].items()}

    print("Region Mapping:", region_mapping)
    print("Available Region Codes:", set(region_mapping.values()))
    # Extract commodity list and arc list from scenario
    commodity_list = sorted(set(c for (_, c) in scenario['demand']))
    print("Commodity List:", commodity_list)
    ...
    redundancy = {int(i): 1 for i in node_list}  # Defines redundancy requirements for each node
    ...
    f_keys = set((i, k) for i in node_list for k in node_list)
    ...
    # Check model status before printing variables
    ...
    for i in node_list:
        for k in node_list:
            if f[k,i].x >0 :
                print(f"\nFacilities {k} for APS at node {i}:")
    # print("\nLeaf node variables:")
    # for j in node_list:
    #     for k in node_list:
    ...
    # Collect and return results, including all tree-related metadata for visualization
    results = {
        "scenario_id": scenario["scenario_id"],
        ...
        "initial_inventory": {(i, c): q[i, c].x for i in node_list for c in commodity_list},
        "w_full": {(i, c): q[i, c].x for i in node_list for c in commodity_list if q[i, c].x > 0},
        "flow_summary": dict(flow_summary),
        "computational_time": computational_time,
        "tree_edges": [
            {"from": i, "to": j, "facility": k}
            for (i, j) in arc_list
            for k in node_list
            if bar_x[i, j, k].x > 0.5
        ],
        "leaf_nodes": [
            {"node": j, "facility": k}
            for j in node_list
            for k in node_list
            if leaf_node_var[j, k].x > 0.5
        ],
        "aps_assignments": [
            {"node": i, "facility": k}
            for i in node_list
