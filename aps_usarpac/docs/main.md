...
## Strategic Context: Humanitarian Logistics in the Indo-Pacific
...
Chapter 2 provides foundation for understanding

# Literature Review

This chapter provides a review of doctrinal and academic concepts
foundational to U.S Joint humanitarian assistance and disaster relief
(HA/DR) logistics planning and operations.

## Joint Doctrine as a Structured Decision System

Joint Force HA/DR operations are governed by a structured doctrinal
framework that defines planning processes, decision points, and
coordination mechanisms necessary to establish a coherent operational
strategy [@JP3-0; @jp329]. The Joint Planning Process (JPP) is the
primary mechanism for translating strategic guidance into operational
plans. Each stage of the JPP produces artifacts such as mission
statements, assumptions, COAs, and decision briefs that frame the
problem, identify risks, and align decision-makers across the staff. The
JPP is a disciplined and effective structure for organizing complex
operational problems.

While JPP explicitly acknowledges uncertainty through assumptions and
risk identification, it does not prescribe how uncertainty should be
formally represented, analyzed, or compared across alternatives.
Consequently, uncertainty is treated implicitly and evaluated primarily
through qualitative judgment rather than systematic methods.

Planning outputs are executed and updated through recurring decision
cycles, or battle rhythms, which govern information collection,
assessment, and decision-making. Within these cycles, logistics planning
relies on running estimates, resource availability forecasts, and demand
updates to maintain awareness of current capabilities and constraints.
These mechanisms support iterative plan refinement as conditions evolve,
but they remain focused on the current state and do not provide a
structured means to evaluate uncertainty across possible future
conditions or to quantify the risk associated with alternative
decisions.

While Joint doctrine establishes the importance of operational
assessments and provides broad mechanisms for conducting assessments,
i.e. measures of performance (MOPs) and measures of effectiveness
(MOEs), it stops short of providing prescriptive analytical
methodologies for evaluating operational or strategic decision making,
especially under conditions of uncertainty. Consequently,
decision-making under uncertainty remains largely experiencedriven and
qualitative, limiting the ability to rigorously assess risk and identify
solutions that are resilient to the range of conditions inherent in
HA/DR operations.

## Stochastic Modeling for Decision-Making Under Uncertainty

Stochastic programming and simulation based methods of analysis enable
decision-making under uncertainty through a well-established set of
tools. These techniques capture uncertainty explicitly and allow
decision-makers to evaluate alternative COAs and decisions across a
defined set of possible outcomes or distributions. They quantify
trade-offs between performance and risk. Stochastic programming, in
particular, allows decisions to be evaluated across multiple scenarios
that represent uncertain future conditions. Using Monte Carlo
simulation, we can generate scenarios that capture variability in an
operational environment such as demand, infrastructure suitability, and
network capacity in HA/DR operations. Optimization models then work to
identify decisions that perform well across these expected scenarios
based expectation or with respect to specific risk measures. While not
comprehensive, these two methods are some of the most common tools for
modeling uncertainty within the operations research and logistics
literature.

These methods have been widely applied in logistics, transportation, and
supply chain planning, where uncertainty plays a central role in system
performance. They perform particularly well in circumstances that
require decision making prior to the realization of uncertain events,
and where system conditions evolve over time. Despite their strengths,
analytical methods of analysis are often developed independently of
operational decision processes. This often creates a gap between the
capabilities offered by stochastic modeling and the way decisions are
actually made within structured planning frameworks such as those
defined by joint doctrine.

## Aligning Stochastic Modeling with Doctrinal Decision Processes

Our research seeks to bridge the gap between the doctrinal planning
process and stochastic modeling capabilities by explicitly aligning
analytical tools with the JPP. Our intent is to provide quantitative
support to the specific components and artifacts that already structure
operational planning in order to complement the existing process. Within
the JPP, COA analysis represents a pivotal stage in which planners must
evaluate alternative strategies under conditions of uncertainty.
Stochastic simulation enables this evaluation by generating plausible
disaster scenarios that reflect variability in demand, infrastructure
degradation, and logistics capacity. These scenarios provide a
structured means of exploring how different COAs perform across a range
of operational conditions, moving beyond single-point assumptions toward
a more comprehensive understanding of risk and variability.

Optimization models support identifying resource allocation and
prepositioning strategies that perform effectively across scenarios.
This directly informs core planning artifacts, such as COA comparisons
and decision briefs, by introducing quantitative measures of
performance, trade-offs, and risk that can be communicated and
scrutinized within the existing planning structure. The temporal
dimension of joint operations is similarly preserved through the use of
discrete time periods, which mirror the decision cycles embedded in
battle rhythms. Each period represents a point at which system
conditions are reassessed and decisions may be adjusted, ensuring that
the analytical framework reflects not only uncertainty in the
environment but also the sequential and adaptive nature of operational
decision-making. In this way, simulation and optimization are not
abstract analytical exercises, but tools deliberately integrated into
the doctrinal process to enhance its effectiveness.

By mapping stochastic modeling components directly to doctrinal
processes, the proposed framework improves both the credibility and
usability of analytical outputs. Simulation provides a disciplined way
to represent uncertainty, optimization offers structured decision
support, and together they contribute meaningfully to the development
and evaluation of planning artifacts. Importantly, this approach is not
intended to replace planner judgment, but to strengthen it by grounding
decisions in transparent, data-informed analysis. The result is a more
robust decision-making process that retains the strengths of doctrinal
planning while enhancing its ability to manage uncertainty, mitigate
risk, and build confidence in complex and dynamic operational
environments.

## Decision Cycles and Time Buckets

Although joint doctrine does not prescribe specific analytical tools for
logistics planning, it does establish recurring operational decision
cycles that structure how commanders and staff evaluate plans and
strategy. Headquarter units operate according to established battle
rhythms that include periodic assessments, logistics updates, and
tasking cycles [@jp30]. These cycles structure how information is
gathered, decisions are evaluated, and resources are allocated.
Humanitarian assistance logistics planning similarly occurs within
recurring operational cycles. Planners continuously assess
infrastructure conditions, transportation capacity, and relief demand as
new information becomes available [@jp329]. Logistics movements such as
airlift scheduling, sealift operations, and ground distribution planning
are typically coordinated within discrete planning intervals rather than
continuously in time.

For this reason, the analytical framework developed in this research
represents time using discrete planning periods. Each period corresponds
to a decision interval during which logistics conditions are reassessed
and distribution plans may be adjusted. This temporal representation
reflects the operational decision cycles described in joint doctrine
while providing a tractable structure for modeling logistics decisions
under uncertainty.

## Disaster Response Optimization Models

## Modeling Uncertainty in Disaster Logistics

## Risk Measures in Stochastic Logistics Planning

Risk measures help quantify decision-maker preferences given uncertainty
within a stochastic model. Models are typically formulated to optimize
the expected value of some objective function given a set of uncertain
scenarios. Expected-value formulations provide valuable insight with
respect to the average performance of a system. However, they are
insufficient when trying to understand impact of extreme plausible
events. We must acknowledge this limitation in the context of disaster
logistics planning and operations since low-probability, high-severity
outcomes significantly affect operational performance. When developing
models to inform these operations, we must incorporate specific
techniques to capture tail risk within the objective function and
modeling constraints. Within the literature, Conditional Value-at-Risk
(CVaR) has emerged as one dominant technique due to its strong
theoretical properties and computational efficiency.

### Traditional Expected Value Formulations

Stochastic optimization models traditionally minimize some expected cost
or maximize respective performance value across a set of uncertain
scenarios. Based on the distribution of random variables, these
formulations determine the mean outcome to account for uncertainty.
These methods provide valuable insight into uncertain system performance
but often perform poorly within the extreme conditions of the
distribution. Since HA/DR response operations must often account for
large-scale infrastructure damage, surges in demand, and severe
transportation disruptions, decision makers cannot afford to marginalize
the tail conditions represented in the underlying distribution. Doing so
introduces the risk of underestimating operational consequences of rare
but high-severity events. This limitation highlights the need to develop
optimization models which are sensitive to risk and explicitly account
for the distribution of severe outcomes.

### Value-at-Risk and Its Limitations

Value-at-Risk (VaR) utilizes a probabilistic threshold on loss as a
means to account for some severe outcomes. For a given confidence level
$\alpha$, VaR defines the maximum loss not to be exceeded, corresponding
to the $\alpha$-quantile of the loss distribution. This provides an easy
to interpret risk measure and gives decision makers a clear benchmark
for defining and assessing acceptable performance.

However, VaR stops at the threshold, failing to measure how bad things
get beyond it. Consequently, two decisions could share the same VaR and
still produce very different outcomes when the system fails. Rockafellar
and Uryasev [@rockafellar2000optimization] explain how this can result
in misleading conclusions, especially in optimization settings. VaR also
lacks convexity in general, which makes it harder to solve and can
produce unstable solutions. This highlights the need to incorporate risk
measurements which capture tail behavior while preserving optimization
tractability.

VaR fails to separate solutions that barely meet the threshold from
those that fail badly beyond it. That gap matters in this setting since
prepositioning decisions must hold up under severe but plausible
disaster scenarios. This limitation motivates the use of Conditional
Value-at-Risk (CVaR), which directly captures tail outcomes and aligns
the model with the objective of limiting extreme humanitarian
consequences.

### Conditional Value-at-Risk

Conditional Value-at-Risk (CVaR) was introduced as a risk measure that
addresses several of the shortcomings associated with VaR. CVaR measures
the expected loss conditional on losses exceeding the VaR threshold. In
other words, CVaR captures the average severity of the worst-performing
outcomes within the tail of the loss distribution.

Rockafellar and Uryasev [@rockafellar2002conditional] provide a
comprehensive theoretical treatment of CVaR for general loss
distributions. Their work establishes the relationship between VaR and
CVaR and demonstrates that CVaR possesses several desirable mathematical
properties that make it well suited for risk-sensitive decision-making.
In particular, CVaR provides a more informative measure of downside risk
by explicitly accounting for the magnitude of extreme losses.

These properties have made CVaR an increasingly popular risk measure in
operations research, finance, and supply chain optimization.

### CVaR in Stochastic Optimization

In addition to its desirable theoretical properties, CVaR can be
incorporated into optimization models in a computationally tractable
manner. Rockafellar and Uryasev [@rockafellar2000optimization]
demonstrate that CVaR minimization can be expressed through an
equivalent convex optimization formulation. This representation
introduces an auxiliary variable and a set of linear constraints that
allow CVaR to be optimized using standard linear programming techniques.

The resulting formulation enables CVaR to be incorporated into
large-scale stochastic optimization problems through sample-average
approximations. This tractability has contributed significantly to the
widespread adoption of CVaR in operations research applications.

Subsequent research has expanded the use of CVaR within a variety of
optimization frameworks. Sarykalin et al. [@sarykalin2008value] provide
a detailed comparison of VaR and CVaR in risk management applications
and highlight the advantages of CVaR in capturing tail risk. Dentcheva
[@dentcheva2006portfolio] explores the integration of risk-sensitive
measures with stochastic dominance constraints in portfolio optimization
problems. Fabian [@fabian2008handling] examines optimization models in
which CVaR appears either as an objective function or as a constraint
controlling downside risk.

More recent research has extended CVaR-based optimization frameworks to
multi-stage and distributionally robust settings. Elci and Noyan
[@elcci2018chance] develop a two-stage stochastic optimization model
that incorporates CVaR within a chance-constrained framework. Wang et
al. [@wang2021two] extend these ideas by integrating CVaR into
distributionally robust optimization models that account for uncertainty
in the underlying probability distributions.

These developments demonstrate the flexibility of CVaR-based
formulations for modeling decision-making under uncertainty across a
wide range of application domains.

### Implications for Theater Sustainment Planning
