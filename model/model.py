...
from model.model_distance_state import solve_distance_state
...
def solve_stochastic_cvar(
    ...
    vif_solve_config: Dict[str, Any] = None,
) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        vehicle_formulation: "aggregate" (default) uses the existing
            ...
            machinery yet (those land in later phases). "distance_state"
            replaces individual labels with integer counts by vehicle type on
            a cumulative-distance-expanded network. Any other value raises
            ValueError.
        ...
        vif_solve_config: Optional staged-solve guidance used when
            vehicle_formulation is "vif" or "distance_state". Supported entries include solver
            parameters, fixed/starting p and b decisions, second-stage MIP
            starts, and local-neighborhood radii. Ordinary VIF solves are
    ...
    if vehicle_formulation == "vif":
        return solve_vif(
            ...
            solve_config=vif_solve_config,
        )

    if vehicle_formulation == "distance_state":
        return solve_distance_state(
            model=model, instance=instance, p=p,
            N=N, PPL=PPL, PPL_set=PPL_set, R=R, Omega=Omega, modes=modes,
            modal_arcs=modal_arcs, modal_arc_cost=modal_arc_cost,
            vehicle_types=vehicle_types, demand=demand, penalty=penalty,
            inventory_if_open=inventory_if_open,
            releasable_fraction=releasable_fraction, site_cost=site_cost,
            selection_budget=selection_budget, P_max=P_max,
